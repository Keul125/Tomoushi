bs.jeux['dck'] = {
	...
	menu: function() {
		...
		outb='<h2>Préparatifs :</h2><ul><li>Staff : 1 au PC, 1 au micro, des graphistes pour dessiner sur leurs tableau à afficher</li><li>Materiel : PC, videoproj, une feuille par équipe donnant la position de leurs bateaux</li></ul><p><span class="bouton" onclick="bs.jeux[\'dck\'].intro2();">Afficher les regles et placer les bateaux</span>'
		
		
		
		bs.public_popup.document.getElementsByTagName('body')[0].style.backgroundImage="url('./dck/image1.png')";
	...
	intro2: function() {
		...
		'<ul>'+
		'<li>Ce jeu se joue à 4 équipes et avec un maximum de 4 joueurs par équipes.</li>'+
		'<li>C’est Sanji qui dessinera des personnages d’animes/manga/JV. \\o/</li>'+
		'<li>Chaque équipe possède 7 navires qu’elle devront positionner dans le tableau selon leur choix avant le début sur une feuille qui leur seront fournis.</li>'+
		'<li>Lorsque une équipe trouve en premier le personnage dessiné, celle-ci gagne le droit de « bombarder » une case.</li>'+
		...
		var outb='<div id="dck_btop_text_id">Placer les bateaux de l\'équipe '+this.configure_curr_equipe+'</div>';
		...
		if(this.configure_curr_equipe<=this.nb_equipes) {
			...
			if(this.board[i][j][1].indexOf(this.configure_curr_equipe)!=-1) {
				...
				if(this.configure_curr_boat==this.nb_bateaux_par_equipe) {
					...
					if(this.configure_curr_equipe<=this.nb_equipes) {
						document.getElementById('dck_btop_text_id').innerHTML='Placer les bateaux de l\'équipe '+this.configure_curr_equipe;
					} else {
		...
		for(var i=1;i<=this.num_rows;++i) {
			...
			for(var j=0;j<=this.num_cols;++j) {
				if(j==0) {
					...
					//affiche un bateau
					if(this.board[i][j][0]==2) {
						imgcell='<img src="./dck/image7.png" style="width:'+(this.cell_width)+'px;position:absolute;">';
					...
					if(this.board[i][j][0]==4) {
						imgcell='<img src="./dck/image7.png" style="vertical-align: bottom;width:'+(this.cell_width)+'px;position:absolute;">';
						for (var b=0;b<this.board[i][j][1].length;++b) {
