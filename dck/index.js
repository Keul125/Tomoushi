bs.jeux['dck'] = {
	...
	d_tdown: function(ev) {
		...
		for (var i=0; i<touches.length; i++) {
			bs.jeux['dck'].d_tmpxmt[touches[i].identifier]=ev._x;
			bs.jeux['dck'].d_tmpymt[touches[i].identifier]=ev._y;

			//bs.jeux['dck'].ongoingTouches.splice(idx, 1, touches[i]);  // swap in the new touch record
		}
	...
	d_tmove: function(ev) {
		...
		if (bs.jeux['dck'].d_started) {
			for (var i=0; i<touches.length; i++) {
				...
				// var idx = bs.jeux['dck'].ongoingTouchIndexById(touches[i].identifier);
				ev._x=touches[i].pageX-touches[i].target.offsetLeft;
				ev._y=touches[i].pageY-touches[i].target.offsetTop;

				
				bs.jeux['dck'].d_context.beginPath();
				bs.jeux['dck'].d_context.moveTo(bs.jeux['dck'].d_tmpxmt[touches[i].identifier], bs.jeux['dck'].d_tmpymt[touches[i].identifier]);
				bs.jeux['dck'].d_context.lineTo(ev._x, ev._y);
				
				
				// console.log (''+bs.jeux['dck'].d_tmpxmt[idx]+' - '+bs.jeux['dck'].d_tmpymt[idx]+' - '+ev._x+' - '+ev._y+'') 
				...
				bs.jeux['dck'].d_context_p.moveTo(bs.jeux['dck'].d_tmpxmt[touches[i].identifier], bs.jeux['dck'].d_tmpymt[touches[i].identifier]);
...
bs.jeux['dck'].d_started=false;
		...
		if (bs.jeux['dck'].d_started && touches.length==0) {
			bs.jeux['dck'].d_started = false;
